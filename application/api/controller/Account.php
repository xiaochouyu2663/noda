...
class Account extends \think\Controller
{
    ...
    public function login(){
        ...
        if(md5($password)==$row['password']){
            db('account')->where('userId',$row['userId'])->setField('loginTime',date('Y-m-d H:i:s'));
    ...
    public function register()
    {
        ...
        $result = $this ->validate([
                    ...
                    'username'  => 'require|regex:^[1][13478][0-9]{9}$|unique:account',
        ...
        /**验证码超时验证 */
        if((time() - strtotime($row['addtime'])) > 60 * 100){
        ...
        $data['userId']=db('account')->insertGetId($data);
         return json_encode(['code'=>200,'msg'=>'注册成功','data'=>[
         	'userId' => $data['userId'],
            'username'=>$username,
            'alias'=>$alias,
            'registTime'=>date('Y-m-d H:i:s'),
    ...
    public function updatePwd(){
        ...
        // 查询用户是否存在
        $row=db('account') -> where('username',$username)
                        -> find();
        if($row==null){
        ...
        db('system_vcode') -> where('id',$rows['id'])
                           -> setField('isUsed',1);
        /**修改用户的密码 */
        db('account') -> where('username',$username)
    ...
    public function upload(){
        ...
        if($file){
            ...
            if($info){
                ...
                db('upload_images') -> insert($data);
                return json_encode(['code'=>200,'msg'=>'上传成功','data'=>[
                    'filename' => $info->getFilename(),
                ...
                // 上传失败获取错误信息
                $error = $file->getError();
                return json_encode(['code'=>400,'msg'=>'上传失败','data'=>$error],JSON_UNESCAPED_UNICODE);
            }
        }
    }
}